...
`rg --files codex_prompts` found 127 prompt files, including 24 First Playable `fp_tasks` prompts and 5 `review_and_qa` prompts.
...
| `MIG-C001_prompt_inventory_and_deprecation.md` | Executed | This inventory and file-top deprecated markers are its output. |
...
| `HFO-C001` to `HFO-C008` | Historical migration source | v0.1 origin/fate/item source; migrate through HFO2/storyline route instead of rerunning blindly. |
...
Historical prompts (`OAG-*`, `DT-*`, `HFO-*`, `BAS-*`) are not deleted or marked deprecated in this pass. Treat them as migration evidence or rerun-only-when-scoped prompts, not as the preferred forward path.
...
This is not automatically a blocker: current `SIM-C00x` and `MIG-C00x` prompts intentionally aggregate many of these families. It becomes a blocker only if a later task asks to execute one of the missing granular prompt files directly.

## Recommended Next Step

After this inventory:

1. Run `MIG-C002_data_registry_unification.md` or `SIM-C002_data_registry_unification.md`, choosing one route explicitly.
2. Keep `MIG-C003` before any character-creation v0.2 implementation.
3. Do not run `CC-*`, `LM-*`, `MLC-*`, or `A18-*` directly.
4. Before touching runtime, resolve duplicate SIM IDs or state the chosen canonical prompt in the task scope.
